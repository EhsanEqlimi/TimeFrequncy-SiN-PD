...
%                     of the faster algorithm (see paper below). Higher
%                     scores are more liberal, lower scores more
%                     conservative. The last number determines the maximal 
%                     number of electrodes that may be interpolated
%                     in a given epoch before it is marked for rejection.
%                     if omitted default values of cfg.thresh=...
%                     [3 3 3 3 3 round(n_electrodes/6)] are used.
%                     !!! extra note on step 3 of FASTER:
%                     this step requires that an ICA has been run on the
%                     dataset and is automatically skipped if no ica weights
%                     are found in the input EEG structure.
%                     If you have run an ICA on your data but you want FASTER
%                     to skip this step, nevertheless, enter a 0 at the third
%                     position in [cfg.thresh] -> [3 3 0 3 3 12].
%       .eyechan    - you may specify indices of eye channels to facilitate
%                     the detection of blink components in step 3
%                     this requires ICA weights (see above)
%                     e.g. cfg.eyechan=[65 66 67 68]; 
...
function [gen_bad_chans,EEG,trials2remove,comps2remove]=eegF_FASTER_OnlyEpoch(cfg,EEG)
comps2remov=[];
...
refc=1;while ~strcmpi(EEG.chanlocs(refc).labels,'Fz');refc=refc+1;end
