...
EDFDir=dir(fullfile(EEGDataPath,'*mast.edf'));%'*corrected.edf'
[ALLEEG, ~, ~, ~] = eeglab;
%% PSD Parameters
Frames=0;
...
TFParam.foi= 1:2:13;% analysis 1 to 13 Hz in steps of 2 Hz
...
for i=1:length(EDFDir) % i-->Subject
    ...
    EEG_S14 = pop_epoch( EEG, {'S 14'} , [0 6], 'newname', 'BDF file resampled epochs', 'epochinfo', 'yes');
    EEG_S14_Removed = pop_eegthresh(EEG_S14,1,[1:size(EEG.data,1)] ,80,200,0,4.5,0,1);%Artifact rejection

    EEG_S16 = pop_epoch( EEG, {'S 16'} , [0 6], 'newname', 'BDF file resampled epochs', 'epochinfo', 'yes');
    EEG_S16_Removed = pop_eegthresh(EEG_S16,1,[1:size(EEG.data,1)] ,80,200,0,4.5,0,1);%Artifact rejection
...
%     [gen_bad_chans,EEG,trials2remove]=eegF_FASTER_OnlyEpoch(cfg,EEG); % run eegF_FASTER function
%     EEG=pop_select(EEG,'notrial',trials2remove); % remove bad epochs
    %% Converting EEGlab data to filedtrip data
    DataS14 = eeglab2fieldtrip(EEG_S14_Removed,'preprocessing','none');
    DataS16 = eeglab2fieldtrip(EEG_S16_Removed,'preprocessing','none');
